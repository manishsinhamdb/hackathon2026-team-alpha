...
**Resumability.** Runs record completed steps and their output references; `resume_run(run_id)` continues from the last completed step. *As built (2026-09-26):* the code orchestrator and the deploy agent implement `continue_run(run_id)` (`mode: "continue"`; deploy also accepts `resume_run`), idempotent and resuming at the first step not done while re-attaching to any in-flight callee. Every durable stage heartbeats `runs.heartbeat_at` (≤ 30 s); a `queued`/`running` run silent for > 3 min is **abandoned** and the Chat Agent marks it `failed` (`error.code ABANDONED`) instead of blocking on it. `chat_retry_stage(poc_id, stage)` (the Control Tower Retry chip, "retry", "continue the build") continues a resumable run in place or starts a fresh one. Long runs **hand themselves over** to a new execution of the same agent at a step boundary after 6 min / 25 tool calls (env-configurable), because the platform kills an execution at ~10 min — see docs/06 "Resilient runs" and docs/08 §8. **Idempotency:** `ClientToken = f"{poc_id}-{run_id}"` on `run_instances`; Atlas cluster name derived from `poc_id`; deterministic S3 keys.
...
*As built (2026-09-26):* when `query_patterns` use `$search`/`$vectorSearch`, the seed also creates every named Atlas Search / Vector Search index (`index: '<name>'`) after the inserts, with a verbatim idempotent `ensureSearchIndexes` helper (create-if-missing, waits until queryable). The validator rejects a seed without it. See docs/08 §8.7.
...
*As built (2026-09-26):* the agent type-checks its own backend (`npm install` + `tsc --noEmit`, bundled Node wheel) and feeds compiler errors back to the model (≤ 3 attempts, 300 s budget). Pipelines with Atlas Search stages must be `Document[]` from `mongodb` run on `Model.collection.aggregate`, never mongoose `PipelineStage[]`; this is a prompt rule, a lint, and a known-fix hint in repair mode.
