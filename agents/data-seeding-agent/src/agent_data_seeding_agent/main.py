"""
Data Seeding Agent — POC Builder Stage 2 sub-agent (Spec §6.4).
...
every reply is an AgentEnvelope response (JSON). Tools:
    generate_seed(poc_id, code_version, mode, inputs, failure?, previous_source_key?)
"""
...
def build_agent() -> CompiledStateGraph:
    ...
    def reply(state: DataSeedingState, response: dict[str, Any]) -> dict[str, Any]:
        return {"messages": [AIMessage(content=json.dumps(response))], "done": True, "result": response}

    def parse_node(state: DataSeedingState) -> dict[str, Any]:
    ...
    def dispatch_node(state: DataSeedingState) -> dict[str, Any]:
        if state.get("done"):
            return {}
        req = state["request"]
        task_id = req["task_id"]
