"""
API Agent — POC Builder Stage 2 sub-agent (Spec §6.5), built on the Agent Engine SDK.
...
every reply is an AgentEnvelope response (JSON). Tool / entry point:
    generate_api(poc_id, code_version, mode, inputs, failure?, previous_source_key?)
    mode "contract" -> api_contract.yaml; mode "code"/"repair" -> the Express backend component.

The agent loads its inputs from S3 itself and writes its output to S3 itself, then replies `succeeded`
with a `contract` artifact (contract mode) or a `code` artifact (backend mode).
...
def build_agent() -> CompiledStateGraph:
    logger.info("Building API Agent graph...")
    ...
    def reply(state: ApiAgentState, response: dict[str, Any]) -> dict[str, Any]:
        return {"messages": [AIMessage(content=json.dumps(response))], "done": True, "result": response}

    def parse_node(state: ApiAgentState) -> dict[str, Any]:
    ...
    def generate_node(state: ApiAgentState) -> dict[str, Any]:
        if state.get("done"):
            return {}
        req = state["request"]
        r = call("api_execute", envelope_json=json.dumps({"request": req}))
        if "error" in r:
            e = r["error"]
            return reply(state, Envelope.failed(req["task_id"], e["code"], e["message"], e.get("retryable", False), e.get("detail")))
        art = Envelope.artifact(r["artifact_kind"], r["artifact_key"], r["code_version"])
        result = {"code_version": r["code_version"]}
        if r.get("component"):
    ...
    logger.info("✅ API Agent graph compiled")
    return graph
