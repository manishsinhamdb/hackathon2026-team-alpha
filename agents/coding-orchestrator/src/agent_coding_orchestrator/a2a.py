...
NOTE: the orchestrator no longer makes A2A calls. It used to run the coders over A2A, but a full 4-coder run
outlives the ~5-min OE A2A token (the last coder 401'd), so coders are now called as TOP-LEVEL platform
invocations via `poc_shared_tools.platform_invoke` (see main.py `coders_node`). The old A2AClient (with its
now-unreachable token-refresh retry) has been removed."""
