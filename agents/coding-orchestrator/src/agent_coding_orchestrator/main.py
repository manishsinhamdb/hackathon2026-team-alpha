"""
Coding Orchestrator — POC Builder Stage 2 (Spec §6.3).
...
KNOWN_TOOLS = {"start_code_run", "repair_component", "get_code_bundle"}
...
class CodingOrchestratorState(_Opt):
...
def build_agent() -> CompiledStateGraph:
    ...
    def reply(state: CodingOrchestratorState, response: dict[str, Any]) -> dict[str, Any]:
        return {"messages": [AIMessage(content=json.dumps(response))], "done": True, "result": response}

    def parse_node(state: CodingOrchestratorState) -> dict[str, Any]:
    ...
    def dispatch_node(state: CodingOrchestratorState) -> dict[str, Any]:
        if state.get("done"):
            return {}
        req = state["request"]
        task_id = req["task_id"]
    ...
    b = StateGraph(CodingOrchestratorState)
