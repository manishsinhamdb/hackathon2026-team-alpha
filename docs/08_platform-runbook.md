...
5. Design limits (from §0), and the rule that follows:
   ...
   **Transport rule (decides A2A vs platform invoke for every cross-agent call):** a **short in-turn hop** (returns within the turn and inside the ~5-min A2A token) stays on **A2A**; **anything that can outlive the turn or the ~5-min A2A token** goes over a **top-level platform invocation** of the callee's workspace with a project **service account** (`agentic service-account create <name> --role AGENT_DEVELOPER --context <ctx>`; token at `POST /api/v1/oauth/token`, client-credentials; invoke at `POST /api/v1/projects/<project>/workspaces/<ws-id>/invoke` with `{message, session_id, user_id}`) — the callee runs in its OWN root session with a freshly-minted token, independent of the caller.
   In this project that rule is implemented by the shared helper **`poc_shared_tools.platform_invoke`** (`invoke_envelope(skill, envelope, …)` for a synchronous caller; `resolve_workspace_id` + `invoke_workspace` for a fire-and-forget start). Cross-agent callers using it: chat → code/deploy/teardown/tests **starts** (fire-and-forget), coding-orchestrator → each coder, and deploy → test/repair (both **synchronous**, 15-min client timeout). The **only** remaining A2A caller is **chat → draft** (short, in-turn). `scripts/check_platform_contract.py` enforces this mechanically.
